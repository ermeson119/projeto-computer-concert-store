...
<body>
<form action="pesquisaraparelho" method="post" class="myForm">
    <c:if test="${dados == null}">
        <fieldset class="formField">
            <legend class="formTitle">Pesquisar Aparelho</legend>
            ...
            <fieldset class="entityField">
                <select class="entitySelect" name="entity" id="entityInput" size="5">
                    <option value="nulo" label="Voltar a pesquisar" style="color: #3a68b7;">
                        <c:forEach items="${dados}" var="aparelho">
                    <option value="${aparelho.id}" label="${aparelho.nome} ${aparelho.modelo} ${aparelho.marca} ${aparelho.numeroDeSerie}"></option>
                    </c:forEach>
                </select>
